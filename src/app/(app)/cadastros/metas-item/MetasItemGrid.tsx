...
export function MetasItemGrid({
  ...
  const total = useMemo(
    () => round2(tipos.reduce((s, t) => s + num(qtd[t.id] ?? '') * Number(t.preco), 0)),
    [qtd, tipos],
  )

  const lavagens = tipos.filter((t) => t.categoria !== 'servico')
  const servicos = tipos.filter((t) => t.categoria === 'servico')
        ...
        <tr key={t.id} className="border-b border-slate-100 last:border-0">
          ...
          <td className="px-4 py-2 text-right">
            <input
              inputMode="numeric"
              value={qtd[t.id] ?? ''}
              onChange={(e) => setQtd({ ...qtd, [t.id]: e.target.value })}
              className="w-24 rounded-md border border-slate-300 px-2 py-1 text-right text-sm"
              placeholder="0"
            />
          </td>
          <td className="px-4 py-2 text-right font-medium text-slate-700">{valor > 0 ? formatBRL(valor) : '—'}</td>
        </tr>
      )
    })
