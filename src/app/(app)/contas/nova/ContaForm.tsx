...
import { criarConta, criarFornecedorRapido, criarTipoDespesaRapido, type ContaState } from '../actions'
...
export function ContaForm({
  ...
  const [tiposLista, setTiposLista] = useState<Opt[]>(tipos)
  ...
  function onNovoTipo(t: Opt) {
    setTiposLista((l) => [t, ...l].sort((a, b) => a.nome.localeCompare(b.nome)))
    setTipoId(t.id)
    setModalTipo(false)
  }
  ...
  return (
    <Card>
      <form action={formAction} className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        ...
        <Field label="Tipo de despesa" htmlFor="tipo_despesa_id">
          <div className="flex gap-2">
            ...
            <button type="button" onClick={() => setModalTipo(true)} className={btnGhost} title="Cadastrar novo tipo de despesa">
              ＋ Novo
            </button>
          </div>
        </Field>
      ...
      {modalTipo && <NovoTipoDespesaModal onClose={() => setModalTipo(false)} onCriado={onNovoTipo} />}
    </Card>
  )
}

function NovoTipoDespesaModal({ onClose, onCriado }: { onClose: () => void; onCriado: (t: Opt) => void }) {
  ...
  function salvar() {
    setErro('')
    if (!nome.trim()) {
      setErro('Informe o nome do tipo de despesa.')
      return
    }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4 text-left">
      <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl">
        <h3 className="mb-4 text-lg font-bold text-brand-dark">Novo tipo de despesa</h3>
        <div className="space-y-4">
          <Field label="Nome" htmlFor="t-nome">
            <input id="t-nome" value={nome} onChange={(e) => setNome(e.target.value)} className={inputClass} placeholder="Ex.: Energia, ISS, Detergente" />
          </Field>
          <Field label="Grupo na DRE" htmlFor="t-grupo" hint="Onde entra na DRE">
            <select id="t-grupo" value={grupo} onChange={(e) => setGrupo(e.target.value)} className={inputClass}>
              {GRUPOS_DRE_OPCOES.map((o) => (
